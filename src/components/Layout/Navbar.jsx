...
const Navbar = ({ activeGroup, setActiveGroup }) => {
    ...
    const handleNavigation = (e, href) => {
        if (href.startsWith('#')) return; // ignore hashes for now as they are on different routes
        e.preventDefault();
        setMobileOpen(false);
        navigate(href);
    ...
    return (
        <nav
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/80 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'
                }`}
        >
